...
export default class ChooseShowing extends React.Component {
	...
	componentDidMount() {
		axios.get('http://35.246.125.69:8000/film/' + this.props.match.params.filmId + '/showings')
		...
		return (
			<div className="times">
				{ _.uniq(_.map(this.state.showings, _.property('time')).map(date => Moment(date).format('YYYY-MM-DD'))).map(day => {
					return <ChooseShowingDaySection className="day" {...this.props} key={day} date={day} showings={this.state.showings.filter((item) => item.time.search(day) !== -1)} />
				})}
			</div>
		);
	}
