	...
	render() {
		return (
			<div className="choosetickets">
				...
				<Link to={{pathname: '/book/chooseseats/' + this.props.match.params.filmId + '/' + this.props.match.params.showingId, state: {film: this.props.location.state.film, showing: this.props.location.state.showing, tickets: this.state}}} onClick={() => this.props.updateTickets(this.state)}><button type='button' className="bookbutton">Book Now</button></Link>
			</div>
		);
	}
}