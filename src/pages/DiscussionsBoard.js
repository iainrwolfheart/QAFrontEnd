...
let alldates = `${year}${separator}${month<10?`0${month}`:`${month}`}${separator}${zerofunction}${date}`
  ...
  refreshPage(){ 
    ...
    componentDidMount() {
      axios.get(`http://10.154.0.3:8000/discussion`)
      .then(response => {
          this.setState({
  ...
  onSubmit = (e) => {
    ...
    filter = new Filter();
    axios.post(`http://10.154.0.3:8000/discussion`, { userName:filter.clean(uname), movieName:filter.clean(movie), comments:filter.clean(comm), rating:rate, datePosted:datepost })
      .then((result) => {
